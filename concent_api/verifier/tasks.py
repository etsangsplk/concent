...
from .utils import get_file_size_and_checksum
...
def blender_verification_order(
    ...
    upload_file_name = generate_upload_file_name(subtask_id)
    (file_size, file_check_sum) = get_file_size_and_checksum(blender_output_file_name)
    ...
    try:
        ...
        upload_file_transfer_token = create_file_transfer_token_for_concent(
            ...
            result_package_hash=file_check_sum,
            operation=message.FileTransferToken.Operation.upload,
        )

        # Upload the image.
        with open(os.path.join(settings.VERIFIER_STORAGE_PATH, blender_output_file_name)) as f:
        ...
        logger.info(f'Uploading blender generated file to storage fails with: {e}')
