...
class SubtaskResultsVerifyIntegrationTest(ConcentIntegrationTestCase):
    ...
    def test_that_concent_should_change_subtask_state_to_failed_if_files_were_not_uploaded_on_time(self):
        ...
        Concent -> Requester: SubtaskResultsSettled
        """

        # given
        (serialized_subtask_results_verify,
         subtask_results_verify_time_str) = self._create_serialized_subtask_results_verify()
        subtask_results_verify_date_time = datetime.strptime(subtask_results_verify_time_str, "%Y-%m-%d %H:%M:%S")

        # when
        with mock.patch("core.message_handlers.core.payments.base.is_account_status_positive", return_value=True):
            with mock.patch("core.message_handlers.send_blender_verification_request"):
